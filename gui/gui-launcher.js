...
命令:
  ...
  katex       启动 KaTeX 检�?GUI (端口 3001)
  merge       启动合并 GUI (端口 3001)
  check       启动编号检�?GUI (端口 3002)
...
`));
      ...
      case 'merge':
        console.log(chalk.red('合并GUI需要单独的服务器文件，请使�? node merge-gui-server.js'));
        process.exit(1);
        break;

      case 'check':
        console.log(chalk.red('编号检查GUI需要单独的服务器文件，请使�? node check-missing-numbers-gui-server.js'));
        process.exit(1);
        break;
