 ...
 * 统一KaTeX渲染错误检测脚本
 * 支持快速模式和详细模式
 * 用法: node katex-check.js <文件夹路径> [选项]
 */
/**
...
function advancedSyntaxCheck(content) {
  ...
  // 检查: 单行数学环境中的多行数学环境
  // 先找到所有的$公式（排除$$），然后检查其中是否包含gather环境
  ...
  // 检查: 单行数学环境中的 aligned 环境
  // 先找到所有的$公式（排除$$），然后检查其中是否包含aligned环境
  if (singleDollarMatches) {
    singleDollarMatches.forEach(match => {
...
function showHelp() {
  ...
  console.log(chalk.white('  --auto-confirm, -y 自动确认所有修复 (与 --auto-fix 配合使用)'));
...
async function main() {
  ...
  try {
    ...
    console.log(chalk.cyan(`🚀 开始KaTeX渲染检查 (${mode}模式)...`));
    if (config.autoFix) {
